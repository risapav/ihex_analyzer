...
class IHex(object):
        ...
        if end is None:
            ...
            for addr, data in self.areas.iteritems():
                if addr >= start:
                    ...
                    result = result[:start] + \
                        data[start-addr:end-addr] + result[end:]
    ...
    def write(self):
            ...
            while i < len(data):
                ...
                if self.mode == 8:
                    ...
                    if newsegbase != segbase:
                        output += self.make_line(0x02, 0,
                                                 struct.pack(">H", newsegbase))
                        segbase = newsegbase
        ...
        if self.start is not None:
            if self.mode == 16:
                output += self.make_line(0x03, 0, struct.pack(">2H",
                                                              self.start[0], self.start[1]))
            elif self.mode == 32:
                output += self.make_line(0x05, 0,
